...
*/
#include <cstdio>
int arr[1001][3001]={};// 그래프
int visited[1001]={};// 방문 배열
int search=0;
char *res="NO";//포인터 문자열
int dfs(int x) {//깊이 우선 탐색
  if(search==x) {
    res="YES";
    return 0;
  }
  visited[x]=1;//방문처리
  for(int i=0;i<3001;i++) {//배열 탐색
    int node = arr[x][i];
    if(!node) continue;//0이라면 그 친구랑 관련이 없음
    if(!visited[i]) dfs(i);//방문한 적 없으면 재귀
  }
}
int main() {
  ...
  for(int i=0;i<m;i++) {
    ...
    arr[x][y]=1;//서로 간선으로 이어줌
    arr[y][x]=1;
  }
  int x; scanf("%d %d",&x,&search);
  dfs(x);//탐색
  printf("%s",res);
  return 0;
}
